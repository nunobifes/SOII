...
int _tmain1(int argc, TCHAR * argv[])
{
	...
	if (argc != 3)
	{
		_tprintf(_T("[Erro]Sintaxe: %s ficheiro_origem ficheiro_destino\n"), argv[0]);
		return -1;
	}
	...
	{
		_tprintf(_T("[Erro] Abrir ficheiros (%d)\n"), GetLastError());
		return -1;
	}
	...
	{
		_tprintf(_T("[Erro]Criar objectos mapeamentos(%d)\n"), GetLastError());
		return -1;
	}
	...
	{
		_tprintf(_T("[Erro]Mapear para memória(%d)\n"), GetLastError());
		return -1;
	}
