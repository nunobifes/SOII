...
// ShowWindow()
int WINAPI WinMain1(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow) {
...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
		...
		_stprintf_s(text_clicks, 20, TEXT("NUM de Clicks = %d"), nClicks);
		hdc = GetDC(hWnd);
		GetClientRect(hWnd, &rect);
		SetTextColor(hdc, RGB(255, 255, 255));
		SetBkMode(hdc, TRANSPARENT);
		rect.left = 5 + nClicks * 80;
		rect.top = 10 + nClicks * 2;
