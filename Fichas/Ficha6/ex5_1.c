...
LRESULT CALLBACK gereEventosDialog(HWND, UINT, WPARAM, LPARAM);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow) {
	...
	wcApp.hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_ICON1));// "hIcon" = handler do ícon normal
												  //"hInst" = Instancia onde o recurso está disponivel
	...
	hWnd = CreateWindow(
		...
		TEXT("ATM - Ficha 6 - Exericio 5.1"),// Texto que figura na barra do título
...
{
	TCHAR login[50], pass[50];
	switch (messg) {
	case WM_COMMAND:
			if(LOWORD(wParam) == IDOK)
			{
				GetDlgItemText(hWnd, IDC_EDIT1, login, 50);
				GetDlgItemText(hWnd, IDC_EDIT2, pass, 50);

				_stprintf_s(frase, 100, _T("%s: %s"), login, pass);
				EndDialog(hWnd, 0);
				return TRUE;
			}
			else if(LOWORD(wParam) == IDCANCEL)
			{
				_stprintf_s(frase, 100, _T(""));
				EndDialog(hWnd, 0);
				return TRUE;
			}
		break;
...
LRESULT CALLBACK TrataEventos(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		{
		case ID_MENU_LOGIN:
			// se o 3o argumento for NULL a janela não é modal
			DialogBox(NULL, MAKEINTRESOURCE(IDD_DIALOG1), hWnd, gereEventosDialog);
			if(_tcscmp(frase, _T("")) != 0)
			{
				MessageBox(hWnd, frase, _T("Login"), MB_OK);
			}
			break;
		case ID_SAIR:
			DestroyWindow(hWnd);
